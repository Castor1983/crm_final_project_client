...
export const editOrderSchema = Joi.object({
  ...
  group: Joi.alternatives().try(Joi.string().min(4).max(20)).messages({
  ...
  phone: Joi.alternatives()
    .try(Joi.string().pattern(/^0\d{9}$/), Joi.valid(null))
    .messages({
      'string.base': 'Phone must be a string or empty',
      'string.pattern.base':
        'Phone must be a valid phone number (e.g., 0123456789)',
    }),

  age: Joi.alternatives()
    .try(Joi.number().integer().min(15).max(100), Joi.valid(null))
    .messages({
