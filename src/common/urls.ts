...
export const urls = {
  ...
  managers: {
    activateManager: (managerId: string) =>
      `${managersUrl}${activate}${manager}/${managerId}`,
    activatePassword: (activateToken: string | undefined) =>
      `${managersUrl}${activate}/${activateToken}`,
    deactivate: (managerId: string) =>
      `${managersUrl}${deactivate}/${managerId}`,
